...
let { company, period, role, description, introduce }: Experience = $props();
...
<div class="relative pl-6 border-l border-(--border)">
	<div class="flex flex-col gap-3 md:flex-row md:justify-between items-start mb-1">
		<h3 class="text-2xl font-bold order-2 md:order-1">{company}</h3>
		<span class="text-sm text-(--description) font-mono order-1 md:order-2">{period}</span>
	</div>
	<p class="mb-3">{role}</p>
	<p class="text-(--description) leading-relaxed mb-4">{description}</p>
	<List>
